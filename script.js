// Enhanced Loading Screen and Starfield Canvas
...
class Star {
    ...
    reset() {
        ...
        this.z = Math.random() * starfieldCanvas.width;
    ...
    update() {
        ...
        if (this.z <= 0) {
            this.reset();
            this.z = starfieldCanvas.width;
        }
    }
    
    draw() {
        const x = (this.x - starfieldCanvas.width / 2) * (starfieldCanvas.width / this.z);
        const y = (this.y - starfieldCanvas.height / 2) * (starfieldCanvas.width / this.z);
        const s = this.size * (starfieldCanvas.width / this.z);
        ...
        if (screenX >= 0 && screenX <= starfieldCanvas.width && 
            ...
            const brightness = 1 - (this.z / starfieldCanvas.width);
            starfieldCtx.fillStyle = `rgba(255, 255, 255, ${brightness})`;
            starfieldCtx.beginPath();
            starfieldCtx.arc(screenX, screenY, s, 0, Math.PI * 2);
            starfieldCtx.fill();
        }
    }
}
...
function animateStarfield() {
    ...
    starfieldCtx.fillStyle = 'rgba(5, 8, 20, 0.2)';
...
async function fetchISSDetails() {
    // This function is no longer needed as we get all data from fetchISSPosition
...
async function fetchSatellites() {
    ...
    // Using ISS as the main satellite since it's the most reliably tracked
    // Other specific satellites may not be available in the API
    const satellites = [
        { name: 'International Space Station', norad: 25544 },
    ];
    ...
    // Add placeholder cards for other satellites
    const placeholderSatellites = [
        { name: 'Hubble Space Telescope', info: 'Orbital data not available' },
        { name: 'Starlink Constellation', info: 'Multiple satellites in LEO' },
        { name: 'James Webb Space Telescope', info: 'Located at L2 point' }
    ];
    
    placeholderSatellites.forEach(sat => {
        const card = document.createElement('div');
        card.className = 'satellite-card fade-in';
        card.innerHTML = `
            <div class="satellite-name">${sat.name}</div>
            <div class="satellite-data">
                <p style="color: var(--gray); text-align: center; padding: 2rem;">
                    ${sat.info}<br><br>
                    Real-time tracking data not available for this satellite.
                </p>
            </div>
        `;
        grid.appendChild(card);
    ...
    if (successCount === 0 && placeholderSatellites.length === 0) {
        grid.innerHTML = '<p style="text-align: center; color: var(--gray);">Unable to load satellite data at this time.</p>';
    }
}
...
async function fetchAsteroids() {
    ...
    try {
        ...
        if (!data.near_earth_objects || Object.keys(data.near_earth_objects).length === 0) {
            container.innerHTML = '<p style="color: var(--gray);">No near-Earth asteroids detected in the next 7 days.</p>';
            return;
        }
        ...
        allAsteroids.slice(0, 5).forEach(asteroid => {
            if (!asteroid.close_approach_data || !asteroid.close_approach_data[0]) return;
            
            const approach = asteroid.close_approach_data[0];
            const item = document.createElement('div');
            item.className = 'asteroid-item fade-in';
            item.innerHTML = `
                <div class="asteroid-name">${asteroid.name || 'Unknown Asteroid'}</div>
                ...
                <div class="asteroid-details">
                    ...
                    Velocity: ${parseFloat(approach.relative_velocity?.kilometers_per_hour || 0).toLocaleString()} km/h<br>
                    Est. Diameter: ${asteroid.estimated_diameter?.meters?.estimated_diameter_min?.toFixed(0) || 'Unknown'} - 
                    ${asteroid.estimated_diameter?.meters?.estimated_diameter_max?.toFixed(0) || 'Unknown'} meters
                </div>
        ...
        container.innerHTML = '<p style="color: var(--gray);">Unable to load asteroid data. NASA API may have rate limits.</p>';
    }
}
// Mars Weather
async function fetchMarsWeather() {
    ...
    // InSight mission has ended, so we'll show a message with historical context
    container.innerHTML = `
        <div class="fade-in" style="padding: 2rem; text-align: center;">
            <p style="color: var(--gray); margin-bottom: 1rem;">
                Mars weather data from NASA's InSight lander is no longer available as the mission concluded in December 2022.
            </p>
            <p style="color: var(--gray); font-size: 0.9rem;">
                The lander successfully operated for over 4 years, providing valuable atmospheric data from Mars' Elysium Planitia region.
            </p>
            <div style="margin-top: 2rem; padding: 1.5rem; background: rgba(112, 0, 255, 0.1); border: 1px solid rgba(112, 0, 255, 0.3);">
                <div style="font-size: 1.2rem; color: var(--accent); margin-bottom: 0.5rem;">Historical Mars Conditions</div>
                <div style="color: var(--light); font-size: 0.9rem;">
                    Typical Temperature: -20°C to -80°C<br>
                    Atmospheric Pressure: ~600-700 Pa<br>
                    Dust Storm Season: September-October (Mars Calendar)
                </div>
            </div>
        </div>
    `;
...
async function fetchSolarEvents() {
    ...
    try {
        ...
        document.getElementById('activeEvents').textContent = totalEvents;
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetchLaunches().catch(err => console.error('Launch fetch failed:', err));
    initISSTracking().catch(err => console.error('ISS tracking failed:', err));
    fetchSatellites().catch(err => console.error('Satellite fetch failed:', err));
    fetchCrewInfo().catch(err => console.error('Crew fetch failed:', err));
    fetchSpacewalks().catch(err => console.error('Spacewalk fetch failed:', err));
    fetchSolarEvents().catch(err => console.error('Solar events fetch failed:', err));
    fetchAsteroids().catch(err => console.error('Asteroid fetch failed:', err));
    fetchMarsWeather().catch(err => console.error('Mars weather fetch failed:', err));
    fetchMeteorShowers();
    fetchMoonPhase();
    fetchAPOD().catch(err => console.error('APOD fetch failed:', err));
...
// Override the initialization to use progressive loading
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        await fetchSpacewalks();
        componentLoaded();
    } catch (err) {
        console.error('Spacewalk fetch failed:', err);
        componentLoaded();
    }
    
    try {
        await fetchSolarEvents();
        componentLoaded();
    } catch (err) {
        console.error('Solar events fetch failed:', err);
        componentLoaded();
    }
    ...
    try {
        ...
        console.error('Asteroid fetch failed:', err);
        componentLoaded();
    }
